#ifndef _HOLBERTON_H_
#define _HOLBERTON_H_
/*function prototypes*/
int _putchar(char c);
int print_int(int num);
int print_string(char *str);
int print_binary(unsigned int num);
int print_hexa(unsigned int num);
int print_hexa_upper(unsigned int num);
int print_octal(unsigned int num);
int print_unsign(unsigned int num);
int print_long_int(long int num);
int print_unsigned_long(unsigned long int num);
int print_long_octal(unsigned long int num);
int long_lower_hex(unsigned long int num);
int long_upper_hex(unsigned long int num);
int print_short_int(short int num);
int print_unsigned_short(unsigned short int num);
int print_short_octal(unsigned short int num);
int short_lower_hex(unsigned short int num);
int short_upper_hex(unsigned short int num);
/*printf prototype*/
int _printf(const char *format, ...);
#endif
